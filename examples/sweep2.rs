...
fn run<T: SizedSample + FromSample<f32>>(
    ...
    // Sweep A: 10kHz → 10Hz
    let mut osc_a = SweepOscillator::new(sample_rate, 10_000.0, 10.0, 10.0);

    // Sweep B: 20kHz → 10Hz
    let mut osc_b = SweepOscillator::new(sample_rate, 20_000.0, 10.0, 10.0);

    let stream = device.build_output_stream(
        config,
        move |data: &mut [T], _| {
            for frame in data.chunks_mut(channels) {
                // Mix both oscillators, scale by 0.5 each to prevent clipping
                let mixed = (osc_a.tick() + osc_b.tick()) * 0.25;
